...
class SpeculationManager(AsyncSpeculationSupport):
    ...
    async def _enqueue_speculative(self, request: Request) -> None:
        ...
        # The full serialized payload (as run.py's entry-request enqueue
        # uses) so a speculative probe keeps every field — body, auth,
        ...
        await self._db.insert_request(request_data)

    async def _ensure_tracked(self, spec_state: SpeculationState) -> None:
        """Create the state's tracking row before its probes are enqueued.

        A probe row carries ``speculation_tracking_id``, so the tracking row
        has to exist first. The upsert is the same one that persists progress
        — running it here just writes the state as it currently stands (fresh
        counters on a first run, the loaded ones on a resume) and records the
        id the probes will point at.
        """
    ...
    async def _persist(self, spec_state: SpeculationState) -> None:
        ...
        spec_state.tracking_id = await self._db.save_speculation_state(
            ...
            seed_value_json=(
                json.dumps(spec_state.seed_value)
                if spec_state.seed_value is not None
                else None
            ),
        )
            ...
            if func_name in self._speculation_state:
                ...
                if param_type is not None:
                    try:
                        ...
                        seed_value_json = saved.get("seed_value_json")
