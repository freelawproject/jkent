...
class PlaywrightTransport(Transport[WorkerPage], Recoverable):
    ...
    def __init__(
        ...
        *,
        ...
        # Rebuild a worker's page every N requests. A page reused for a whole
        # run accumulates per-navigation network state that never drains
    ...
    async def acquire(self, worker_id: int) -> WorkerPage:
        ...
        if (
            ...
            logger.info(
                "Worker %d page recycled after %d requests",
                worker_id,
                handle.uses,
            )
    ...
    async def _resolve(
        ...
        # for the whole navigation, not one commit per subresource.
        if handle.incidental_requests:
            await self._db.insert_incidental_requests(  # type: ignore
        ...
        *,
        ...
        if compression_dict_id is not None:
            ...
            dictionary = await get_dict_by_id(
                self._db._session_factory,  # type: ignore[union-attr]
                compression_dict_id,
                db_lock=self._db._lock,  # type: ignore[union-attr]
            )
    ...
    def should_restart(self, exc: BaseException) -> bool:
        ...
        A crashed *content process* ("Page crashed" / "Target crashed") counts
        too. Such a page is not ``is_closed()``, so ``acquire`` would otherwise
        hand it back and only discover the damage when ``reset_for_reuse``
        fails — logging a generic reset warning that reads like a navigation
        race rather than a crash. Naming it here makes a real crash
        distinguishable and rebuilds the handle at the point of failure.
        """
        msg = str(exc)
        return (
