...
class RequestQueue(RequestQueueDB):
    ...
    async def enqueue_request(
        ...
        if request_data.dedup_key and await self.db.check_dedup_key_exists(
            request_data.dedup_key
        ):
            return

        await self.db.insert_request(request_data, skip_dedup_check=True)
