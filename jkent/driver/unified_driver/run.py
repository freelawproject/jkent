...
class ScrapeRun(AsyncLifecycle):
    ...
    The worker pool is pinned: ``num_workers`` workers are spawned once and
    never replaced — a worker retires only when the queue is drained
    (nothing pending, nothing in flight), so the pool cannot collapse while
    work can still appear. Spawning is immediate by default;
    ``worker_ramp_interval`` staggers it instead, which changes only how fast
    the pool fills, not its size or its pinned-ness. ``open``/``aclose`` (from
    ...
    ``db`` injects an already-open :class:`SQLManager` instead of building
    one. Callers that hand a browser transport its own DB handle must pass
    that same manager here: every writer on a SQLite file has to share one
    manager, or the per-manager write lock serializes nothing and concurrent
    writers collide. An injected manager is borrowed — the caller disposes
    its engine.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Seconds between worker spawns at startup. 0 (the default) spawns the
        # whole pool at once, as before. A positive value staggers it: worker 0
        # starts immediately and one more joins per interval, so N workers do
        # not open N browser pages / hit the origin simultaneously on the first
        # tick. The final pool size is unchanged — this shapes arrival, not
        # capacity, and the pool stays pinned once ramped.
        self.worker_ramp_interval = worker_ramp_interval
        ...
        # An injected manager is borrowed, not owned: the caller opened the
        # engine and closes it. Sharing one manager (and therefore one write
        # lock and one connection pool) is what keeps a browser transport's
        # incidental writes from racing the run's own writes on the same
        # SQLite file — two handles mean two locks, which serialize nothing.
        self._db: SQLManager | None = db
        self._engine: Any | None = db.engine if db is not None else None
    ...
    def _start_pool(self) -> None:
        """Bring the worker pool up, all at once or on a ramp.

        With ``worker_ramp_interval`` unset the whole pool is spawned inline,
        exactly as before. With a ramp, worker 0 still starts synchronously —
        so the run is never momentarily worker-less — and the remainder are
        spawned by :meth:`_ramp_workers` in the background, which
        :meth:`_drain_workers` watches alongside the workers themselves.
        """
        ...
        if self.num_workers > 1:
            self._ramp_task = asyncio.create_task(self._ramp_workers())

    async def _ramp_workers(self) -> None:
        """Add one worker per ``worker_ramp_interval`` until the pool is full.
        ...
        for _ in range(self.num_workers - 1):
            with contextlib.suppress(asyncio.TimeoutError):
                await asyncio.wait_for(
                    ...
                    timeout=self.worker_ramp_interval,
                )
            if self.stop_event.is_set() or not self._worker_tasks:
                return
            self.spawn_worker()
    ...
    async def run(self) -> None:
        ...
        with lenient_te_for(self.scraper):
            self._started = True
            await self._db.update_run_status(RunStatus.RUNNING)  # type: ignore[misc]
    ...
    async def _enqueue_entry_request(
        ...
        # Entry-point requests are ordinary queue rows with no parent.
        data = queue.serialize_request(entry_request)
        data.priority = entry_request.effective_priority
        data.dedup_key = (
            entry_request.deduplication_key
            if isinstance(entry_request.deduplication_key, str)
            else None
        )
        await db.insert_request(data)
    ...
    async def _drain_workers(self) -> None:
        ...
        swallowed as an unretrieved task exception.
        """
        while True:
