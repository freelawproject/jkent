...
class PoolWorker:
    ...
    * collaborators: ``_transport``, ``_storage``, ``_continuation`` (and
    ...
    # Only re-stamp started_at when the breaker/rate gates actually held the
    # request at least this long. The restamp exists so DB-derived durations
    # exclude gate waits; when the gates return immediately (tokens available,
    # bypass, replay — the common case) the dequeue stamp is already honest
    # and the restamp would be a pure extra write transaction per request.
    ...
    async def _execute_one(
        ...
        # a rate-limiter token (started_at was stamped at dequeue) — but
        # only when the gates actually waited; an immediate pass-through
        # leaves the dequeue stamp accurate and skips the write.
        if time.monotonic() - gate_started >= self.RESTAMP_MIN_GATE_WAIT_S:
