...
class ContinuationExecutor:
    ...
    async def _stage_preresolved_incidentals(
        ...
        for rec in records:
            ...
            if spec.body_contains is not None and rec.storage_id is not None:
                ...
                body = storage["body"] if storage else None
            if spec.matches(
                url=rec.url,
                method=rec.method,
                headers=headers,
    ...
    async def _build_preresolved_response(
        ...
        if storage is not None:
            stored_status = storage["status_code"]
            if stored_status is not None:
                status_code = stored_status
            headers_json = storage["response_headers_json"]
            content_compressed = storage["content_compressed"]
            content_size_original = storage["content_size_original"]
            content_size_compressed = storage["content_size_compressed"]
            compression_dict_id = storage["compression_dict_id"]
