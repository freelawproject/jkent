...
(``xdotool``), the only input Turnstile accepts.
...
the topmost receives pointer input.
...
def supported() -> bool:
    ...
    Needs Xvfb to run the server and xdpyinfo (``x11-utils``, packaged
    separately) to tell when it is ready.
    """
    return (
        sys.platform.startswith("linux")
        and which("Xvfb") is not None
        and which("xdpyinfo") is not None
    )
...
class XvfbDisplay:
    ...
    def start(self) -> str:
        ...
        One display per ``XvfbDisplay`` is all that is supported for now: a
        second ``start()`` before :meth:`stop` raises.

        Raises:
            RuntimeError: if this display is already started, Xvfb is
                unavailable, or no display number in the search range is
                free.
        """
        if self._proc is not None:
            raise RuntimeError(
                f"XvfbDisplay already started on {self._display}; stop() it "
                "first"
            )
        if not supported():
            raise RuntimeError(
        ...
        for number in range(_SEARCH_START, _SEARCH_END):
            ...
            if Path(f"/tmp/.X{number}-lock").exists():  # X's own path
                continue
            ...
            try:
                ready = self._wait_ready(proc, display)
    ...
    def stop(self) -> None:
        ...
        try:
            ...
            try:
                ...
                proc.kill()
                proc.wait(timeout=5)
        except Exception:  # teardown must not mask a real error
