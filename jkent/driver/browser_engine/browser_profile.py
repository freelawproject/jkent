...
browser — enabling persistent contexts and init scripts that run before
page JavaScript.
...
Security:
    ...
    - Script paths are resolved and checked against the profile directory,
      and ``name`` must be a single path component, to prevent path
      traversal.
"""
...
def load_browser_profile(profile_path: Path) -> BrowserProfile:
    ...
    # ``name`` is a directory under the run's browser data
    # (``resolve_user_data_dir``), so it must be exactly one component.
    if name in (".", "..") or "/" in name or "\\" in name:
        raise ValueError(
            f"manifest.json 'name' must be a single path component: {name!r}"
        )
    ...
    if raw.get("protocol_params"):
        # Rejected rather than ignored: launching without the stealth params
        # the manifest asks for would fail silently.
        raise ValueError(
            "protocol_params is no longer supported; remove it from "
            f"{manifest_path}"
        )
