...
class RequestQueueDB:
    ...
    # Class-level default; instances mutate it into an instance attribute.
    # Subclass __init__s don't chain here, so a plain class default keeps
    # every subclass covered without touching their constructors.
    _in_flight: int = 0
    ...
    async def _prepare_enqueue(
        ...
        already carries the effective ``priority``, ``dedup_key``, and
        ``parent_id``, ready for ``insert_request`` / ``stage_request``.
        """
        ...
        request_data.dedup_key = dedup_key
    ...
    def serialize_request(
        ...
        ``priority``, ``dedup_key``, and ``parent_id`` are left at their
        defaults — the enqueue paths fill them in.
        ...
        raw_continuation = request.continuation
        continuation: str = (
            raw_continuation
            if isinstance(raw_continuation, str)
            else getattr(raw_continuation, "__name__", repr(raw_continuation))
        )
    ...
    async def get_next_request(
        ...
        Returns:
            Tuple of (request_id, request, parent_request_id, preresolved)
            or None if queue is empty. ``preresolved`` is True when the row
            already carries its response (promoted from a captured incidental)
            and the worker should skip the transport.
    ...
    def _deserialize_request(self, row: DequeuedRow) -> Request:
        ...
        verify_raw = row.verify
        via_json_raw = row.via_json
        bypass_rate_limit_raw = row.bypass_rate_limit
        deduplication_key_raw = row.deduplication_key
        timeout_json_raw = row.timeout_json
        json_data_raw = row.json_data
        files_json_raw = row.files_json
        auth_json_raw = row.auth_json
        allow_redirects_raw = row.allow_redirects
        proxies_json_raw = row.proxies_json
        stream_raw = row.stream
        cert_json_raw = row.cert_json
        archive_hash_header_raw = row.archive_hash_header
        reseedable_raw = row.reseedable
        ...
        accumulated_data: dict[str, Any] = (
            json.loads(row.accumulated_data_json)
            if row.accumulated_data_json
            else {}
        )
        permanent: dict[str, Any] = (
            json.loads(row.permanent_json) if row.permanent_json else {}
        )
        ...
        via: Any = None
        if via_json_raw:
            via = via_from_json(via_json_raw)
        ...
        if request_type == RequestType.ARCHIVE:
            return Request(
                **common,
                archive=True,
                expected_type=expected_type,
        ...
        elif request_type == RequestType.NON_NAVIGATING:
