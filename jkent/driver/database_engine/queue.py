...
class RequestQueueDB:
    ...
    async def get_next_request(
        ...
        request_id = row[0]
        parent_request_id = row[30]
        preresolved = bool(row[31])  # trailing RETURNING column

        # Deserialize using the first 30 columns (excluding parent_request_id
        # at [30] and preresolved at [31]).
        request = self._deserialize_request(row[:30])
        return (request_id, request, parent_request_id, preresolved)
