...
class ErrorRecord:
    ...
    traceback: str | None
    # Optional to match the column: ``errors.created_at`` is nullable, and its
    # server default only fires when the INSERT omits it. In practice every
    # row has one — ``to_json`` has always guarded the None anyway.
