...
class ResponseStorageDB:
    ...
    retry_jitter: float  # type: ignore
    ...
    async def load_preresolved_response(
        ...
        if stored is None:
            return None
        status_code, headers_json, url, compressed, dict_id = stored
        if compressed:
            content = await decompress_response(
                self.db.session_factory, compressed, dict_id
            )
        else:
            content = b""
    ...
    async def handle_retry(
        ...
        and floored at :data:`MIN_RETRY_DELAY_S`.
        ...
        if is_archive:
            ...
            if content_size_original > 0:
                compressed, dict_id = await compress_response(
                    self.db.session_factory, content, continuation
                )
    ...
    def _serialize_result_for_storage(
        ...
        # default=str throughout: the invalid-result path feeds this a raw
        # ``failed_doc`` holding arbitrary scraped values (dates, Decimals,
        # …). Raising TypeError here would escape the generator loop and
        # replace the DataFormatAssumptionException the caller is trying to
        # record, losing message/selector/model_name/validation_errors and
        # misfiling the error as unknown/builtins.TypeError. Mirrors the
        # same guard in database_engine.errors.build_error.
        if hasattr(data, "model_dump"):
            data_json = json.dumps(data.model_dump(mode="json"), default=str)
        elif hasattr(data, "dict"):
            data_json = json.dumps(data.dict(), default=str)
        else:
