...
class _StagedRequest:
    ...
    ``request_data`` carries its own ``dedup_key`` and ``parent_id``.
    """
...
class StagedWrites:
    ...
    def stage_request(
        ...
        *,
        ...
        Intra-step dedup: a second yield with the same
        ``request_data.dedup_key`` as one already in this buffer is a no-op.
        Cross-step dedup against rows already committed is checked at flush
    ...
    async def flush(
        ...
        *,
        ...
        async with db._write_session() as session:
            ...
            staged_keys = [
                q.request_data.dedup_key
                for q in self.requests
                if q.request_data.dedup_key is not None
            ]
            existing_keys = (
            ...
            for q in self.requests:
                dedup_key = q.request_data.dedup_key
                if dedup_key is not None and dedup_key in existing_keys:
                    if q.preresolved_response is not None:
                        # Same loudness as the insert-level dedup: skipping a
                        # pre-resolved request drops a captured response and
                        # its continuation, not just a redundant fetch.
                        logger.warning(
                            "Staged pre-resolved request for %s deduplicated "
                            "against an existing row (dedup key %r); the "
                            "captured response was dropped and its "
                            "continuation will not run.",
                            q.request_data.url,
                            dedup_key,
                        )
                    continue
