...
This module defines all database table models using SQLModel.
These models are used with SQLAlchemy's async engine for all
database operations.
...
class Request(SQLModel, table=True):  # type: ignore[call-arg]
    ...
    # subtree (self-referential) in one statement — used for replay pruning.
