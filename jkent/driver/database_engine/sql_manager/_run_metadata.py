...
class SpeculationConfigEntry(TypedDict):
    """Per-continuation speculation tuning, as stored in run metadata."""

    #: Consecutive probe failures past ``highest_successful_id`` before
    #: speculation for the continuation stops.
    threshold: int
    #: How many ids to probe ahead of the highest successful one.
    speculation: int
...
class RunMetadataDict(TypedDict):
    ...
    The ``*_json`` columns come back decoded (``speculation_config``,
    ``browser_config``, ``seed_params``); ``None`` where nothing was stored.
    """
    ...
    speculation_config: dict[str, SpeculationConfigEntry] | None
...
class RunMetadataMixin(SQLManagerBase):
    ...
    async def init_run_metadata(
        ...
        max_backoff_time: float,
        jitter: float = 0.0,
        speculation_config: dict[str, SpeculationConfigEntry] | None = None,
        ...
        The JSON config columns are deliberately *not* re-stamped.
        ``seed_params_json`` is merge-managed (``add_seed_params`` folds
        new entries into the stored list and writes it back through
        :meth:`update_seed_params`), and ``speculation_config`` /
        ``browser_config`` are not passed by the run path at all, so
        overwriting them on resume would blank whatever the first run
        stored.
        ...
        async with self._write_session() as session:
            result = await session.execute(
                select(RunMetadata.id).where(RunMetadata.id == RUN_METADATA_ID)
            )
    ...
    async def get_seed_params(
        ...
        async with self.session_factory() as session:
            result = await session.execute(
                select(RunMetadata.seed_params_json).where(
    ...
    async def get_run_metadata(self) -> RunMetadataDict | None:
        """Get run metadata from database.

        Returns:
            :class:`RunMetadataDict` or None if not found.
        """
        async with self.session_factory() as session:
            result = await session.execute(
                select(RunMetadata).where(RunMetadata.id == RUN_METADATA_ID)
            )
            ...
            return {
                ...
                "seed_params": (
                    json.loads(row.seed_params_json)
                    if row.seed_params_json
                    else None
                ),
            }
