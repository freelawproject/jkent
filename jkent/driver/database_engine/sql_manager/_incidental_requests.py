...
def incidental_record_select() -> Select[Any]:
    ...
    :func:`row_to_incidental_record` maps the row by column name, so the
    selected set — not its order — is the contract between the two.
    """
    return select(
...
def row_to_incidental_record(row: Row[Any]) -> IncidentalRequestRecord:
    ...
    return IncidentalRequestRecord(
        ...
        from_cache=(
            bool(m["from_cache"]) if m["from_cache"] is not None else None
        ),
        ...
        content_size_original=m["content_size_original"],
        content_size_compressed=m["content_size_compressed"],
...
class IncidentalRequestStorageMixin(SQLManagerBase):
    ...
    async def insert_incidental_request(
        ...
        ``response_headers_json`` is deliberately *not* part of the match:
        volatile headers (``Date``, ``ETag``) differ on nearly every response,
        so keying on them would defeat the dedup entirely. A deduplicated
        capture therefore stores the first capture's headers — treat stored
        response headers as representative, not exact (a promotion via
        ``PreresolvedResponse`` replays the first capture's headers).
    ...
    async def insert_incidental_requests(
        self,
        parent_request_id: int,
        records: list[IncidentalRequestParams],
    ) -> list[int]:
        ...
        *,
        ...
        if content_compressed is not None:
            content_md5 = hashlib.md5(
                content_compressed, usedforsecurity=False
            ).digest()
    ...
    async def get_incidental_request_storage(
        ...
        async with self.session_factory() as session:
            ...
            return {
                ...
                # Hex, not raw bytes — see IncidentalStorageDict.
                "content_md5": (
