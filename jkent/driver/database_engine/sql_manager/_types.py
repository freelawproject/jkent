...
def compute_cache_key(
    ...
    The cache key is an MD5 hash of the request parameters that affect
    the response: method, URL, body, and headers. MD5 is chosen for its
    digest width, not its strength — the key is a per-run lookup that never
    leaves the run database, and is not a trust boundary.
