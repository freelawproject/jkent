...
class _DictCache:
    ...
    dictionary blob under the shared DB lock and rebuilds a
    ``ZstdCompressionDict`` from it.
    ...
    continuation to its newest ``(dict_id, dict)`` and also caches the
    ...
    def __init__(self) -> None:
        ...
        self.latest: dict[
            str, tuple[int, zstd.ZstdCompressionDict] | None
        ] = {}


# Keyed weakly by session factory: one cache per open database, dropped with
# the factory. Two databases can reuse the same dict_id with different bytes,
# so a process-global id keyed cache would be wrong.
_dict_caches: WeakKeyDictionary[async_sessionmaker, _DictCache] = (
    WeakKeyDictionary()
)


def _cache_for(session_factory: async_sessionmaker) -> _DictCache:
    cache = _dict_caches.get(session_factory)
    if cache is None:
        cache = _dict_caches[session_factory] = _DictCache()
    return cache


def _as_zstd_dict(
    dictionary: bytes | zstd.ZstdCompressionDict,
) -> zstd.ZstdCompressionDict:
    """Wrap raw dictionary bytes; pass an already-built object through."""
...
def compress(
    ...
    dictionary: bytes | zstd.ZstdCompressionDict | None = None,
) -> bytes:
    ...
    Args:
        ...
        dictionary: Optional pre-trained dictionary for better compression,
            as raw bytes or an already-built ``ZstdCompressionDict``.
    ...
    if dictionary:
        compressor = zstd.ZstdCompressor(
            level=level, dict_data=_as_zstd_dict(dictionary)
        )
    else:
...
def decompress(
    ...
    Args:
        ...
        dictionary: Dictionary used for compression (must match), as raw
            bytes or an already-built ``ZstdCompressionDict``.
    ...
    if dictionary:
        decompressor = zstd.ZstdDecompressor(
            dict_data=_as_zstd_dict(dictionary)
        )
    else:
        decompressor = zstd.ZstdDecompressor()
...
async def get_compression_dict(
...
) -> tuple[int, zstd.ZstdCompressionDict] | None:
    ...
    Returns:
        Tuple of (dict_id, dictionary) or None if no dictionary exists.
    """
...
async def get_dict_by_id(
    ...
    dict_id is an error path, not a steady state.
...
async def recompress_responses(
    ...
    for request_id, compressed, old_dict_id in rows:
        try:
            # get_dict_by_id is cached per-database, so a shared old_dict_id
            # (the common case for a single continuation) is fetched once.
            old_dictionary = (
                await get_dict_by_id(
        ...
        async with lock, session_factory() as session:
            for (
                ...
                result = await session.execute(
                    ...
                    .where(
                        Request.id == req_id,
                        Request.content_compressed == old_compressed,
                    )
                    .values(
