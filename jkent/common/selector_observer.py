...
class SelectorQuery:
    ...
    # Upward backref for tree navigation; excluded from serialization at every
    # depth. A plain ``exclude={"parent"}`` only drops the top object's parent,
    # leaving nested children's backrefs to form a cycle -- Field(exclude=True)
    # applies per-field recursively, so the serializer never walks the cycle.
    parent: Annotated[SelectorQuery | None, Field(exclude=True)] = None

    def to_dict(self) -> dict[str, Any]:
        """Convert to dictionary for JSON serialization.

        Returns:
            Dictionary representation of the query (without ``parent``).
        """
        return _QUERY_ADAPTER.dump_python(self, mode="json")


# Built once: reused by every SelectorQuery.to_dict call.
_QUERY_ADAPTER: TypeAdapter[SelectorQuery] = TypeAdapter(SelectorQuery)
...
class SelectorObserver:
    ...
    def __init__(self, max_sample_length: int = 100, max_samples: int = 3):
        ...
        # A stack, not a single token: the same observer can be re-entered.
        # The @step wrapper activates its observer around every generator
        # resume, and when that observer was injected by an outer ``with``
        # block (see get_active_observer() reuse in decorators), the inner
        # activation nests inside the outer one of the same instance. A lone
        # token slot would be clobbered by the inner enter, leaving the outer
        # exit unable to reset the contextvar.
