...
class TestEntryMetadata:
    ...
    def test_speculative_protocol_detected(self):
        """Pydantic models structurally satisfying Speculative are detected."""
        # ``isinstance/issubclass(X, Speculative)`` is unusable: the Protocol
        # carries a non-method attribute (``should_advance``) and is not
        # ``@runtime_checkable``. ``_implements_speculative`` is the structural
        # check the @entry machinery uses to detect speculation params.
        assert _implements_speculative(RecordId)
        assert not _implements_speculative(OpinionFilters)
