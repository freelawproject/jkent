...
FIELD_VALUES: dict[str, Any] = {
    ...
    "is_speculative": True,
    "speculation_tracking_id": 3,
    "speculative_index": 7,
    "via": ViaLink(
        selector=Selector.XPath("//a[1]"), description="link: Case"
    ),
    "incidental": Singular(url="*casedetaildata*"),
...
def test_every_field_survives_speculative():
    original = make_full_request()
    spec = original.speculative(9, 2024)
    ...
    assert spec.speculation_tracking_id == 9
    assert spec.speculative_index == 2024
