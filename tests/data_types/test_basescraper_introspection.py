...
class TestHttpOverrideDisjointness:
    """Overlapping HTTP status overrides fail at class-definition time.

    A code in two override sets is subtracted from every active set and
    classifies as nothing, so it must be rejected when the class is defined.
    """

    def test_overlapping_overrides_raise(self):
        with pytest.raises(ScraperConfigError, match=r"\b429\b"):

            class BadScraper(BaseScraper[dict]):
                TRANSIENT_HTTP_ERROR_CODES = frozenset({429})
                PERSISTENT_HTTP_ERROR_CODES = frozenset({429})

    def test_disjoint_overrides_are_fine(self):
        class GoodScraper(BaseScraper[dict]):
            SUCCESSFUL_HTTP_CODES = frozenset({450})
            TRANSIENT_HTTP_ERROR_CODES = frozenset({429})
            PERSISTENT_HTTP_ERROR_CODES = frozenset({430})

        # The override moved 429 into transient without it vanishing.
        assert 429 in GoodScraper.active_transient_http_error_codes()
        assert 450 in GoodScraper.active_successful_http_codes()
