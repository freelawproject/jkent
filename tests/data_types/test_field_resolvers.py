...
def test_form_submit_carries_resolver_in_both_payload_and_via():
    ...
    request = _search_form().submit(
        data={"q": "smith", "captcha": resolver}, step="parse"
    )
...
async def test_resolve_substitutes_get_params():
    ...
    request = _search_form(method="GET").submit(
        data={"captcha": resolver}, step="parse"
    )
...
async def test_resolver_may_return_a_list():
    ...
    request = _search_form().submit(data={"captcha": resolver}, step="parse")
...
async def test_resolve_substitutes_pair_list_payloads(field: str):
    """A resolver in a pair list is awaited like one in a dict.

    The scan covered dicts only, so the resolver's repr went on the wire
    and into the dedup key.
    """
    resolver, calls = _counting_resolver("solved")

    def build(value: Any) -> Request:
        pairs = [("q", "a"), ("captcha", value), ("q", "b")]
        return Request(
            request=HTTPRequestParams(
                url="https://example.com/api",
                method=HttpMethod.POST,
                params=pairs if field == "params" else None,
                data=pairs if field == "data" else None,
            ),
            step="parse",
        )

    resolved = await build(resolver).resolve_deferred_fields()

    assert getattr(resolved.request, field) == [
        ("q", "a"),
        ("captcha", "solved"),
        ("q", "b"),
    ]
    ...
    assert (
        ...
        == build("solved").effective_deduplication_key
    )


async def test_no_resolvers_returns_self():
    request = _search_form().submit(data={"q": "smith"}, step="parse")
...
async def test_auto_dedup_key_regenerated_from_resolved_values():
    ...
    concrete = _search_form().submit(
        data={"q": "smith", "captcha": "solved"}, step="parse"
    )
    ...
    assert (
        resolved.effective_deduplication_key
        != request.effective_deduplication_key
    )
    assert (
        resolved.effective_deduplication_key
        == concrete.effective_deduplication_key
    )
...
async def test_sync_callable_rejected():
    request = _search_form().submit(
        data={"captcha": lambda: "solved"},  # type: ignore[dict-item, return-value]
        step="parse",
    )
...
async def test_resolver_exception_propagates():
    ...
    request = _search_form().submit(data={"captcha": resolve}, step="parse")
...
async def test_prepare_enqueue_serializes_resolved_values():
    ...
    # No DB needed with parent_id: (de)serialization only.
    queue = RequestQueue(None)  # type: ignore[arg-type]
    request_data, _ = await queue._prepare_enqueue(
    ...
    assert request_data.via_json is not None
    assert '"captcha":"solved"' in request_data.via_json
    assert "resolve" not in request_data.via_json
    # The key is the resolved request's, as if the value had been concrete.
    concrete = _search_form().submit(
        data={"q": "smith", "captcha": "solved"}, step="parse"
    )
    assert request_data.deduplication_key == (
        concrete.effective_deduplication_key
    )
