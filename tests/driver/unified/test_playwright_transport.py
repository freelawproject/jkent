            ...
            await session.execute(
                sa.text(
                    ...
                    INSERT INTO requests (
                        ...
                        compression_dict_id)
                    VALUES (:status, 9, 1, :method, :url, 'parse', '', 200,
...
def test_should_restart_recognizes_each_dead_message() -> None:
    """Every dead-resource message is recognized as death.

    The last two are content-process crashes rather than dead connections. A
    crashed page is NOT ``is_closed()``, so without them ``acquire`` hands the
    wreck back and the damage only surfaces as a generic ``reset_for_reuse``
    warning that reads like a navigation race — which is what made a suspected
    camoufox tab crash indistinguishable from an about:blank collision.
    """
...
async def test_resolve_goto_honors_request_timeout() -> None:
    ...
    A ``(connect, read)`` tuple uses the read element; an explicit ``None``
    passes no ``timeout`` kwarg so Playwright's default applies. An unset
    timeout gets the ``HTTPRequestParams`` default (60 s).
    """
    ...
    cases: tuple[tuple[dict[str, Any], float | None], ...] = (
        ({"timeout": 45}, 45000.0),
        ({"timeout": (5, 90)}, 90000.0),
        ({"timeout": None}, None),
        ({}, 60000.0),  # unset -> HTTPRequestParams default
    )
...
class TestPlaywrightTransportClassification(ClassificationConformance):
    ...
    # pyre-ignore[13]: bound by the autouse ``_bind_env`` fixture before
    # every test, which pyre can't see.
    ...
    async def classify(
        ...
        *,
        ...
        request_id = await _insert_request_row(
            self._sf, f"{server.base_url}/page", qc=1
        )
        transport = PlaywrightTransport(
...
async def _resolve_with_blocking(
    ...
    async def png_handler(_request: web.Request) -> web.Response:
        hits.append("/pic.png")
        # 1x1 transparent GIF bytes are fine; only the request matters.
        return web.Response(
...
async def test_page_recycled_after_configured_uses(
    ...
    """The worker's page is rebuilt once it has served N requests."""
    if not has_browser:
        pytest.skip("no launchable browser engine in this environment")
...
async def test_page_recycling_preserves_session_cookies(
    ...
    try:
        ...
        rid = await _insert_request_row(memory_session_factory, set_url, qc=1)
        ...
        rid2 = await _insert_request_row(
            memory_session_factory, echo_url, qc=2
        )
        await transport.resolve(
            recycled,
