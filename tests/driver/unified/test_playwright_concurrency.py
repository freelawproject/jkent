"""Concurrency tests for ``PlaywrightTransport``.

Worker-page registry invariants: ``acquire``/``release`` keep a 1:1,
...
Concurrent DOM-snapshot / incidental isolation: under forced
...
# --- Worker-page registry invariants -------------------------------------


@pytest.fixture
async def registry_transport(require_browser: None):
...
async def _release_all(transport: PlaywrightTransport) -> None:
    """Start a Hypothesis example from an empty registry.

    The transport fixture is function-scoped, so it outlives every example
    ``@given`` draws; a handle left by one example would otherwise be the
    next one's starting state, and a shrunk case would not reproduce alone.
    """
    for wid in list(transport._handles):
...
# --- Crash → rebuild (browser-free, via the recovery override) -----------
...
# --- Concurrent snapshot / incidental isolation --------------------------
...
async def test_concurrent_resolves_isolate_snapshot_and_incidentals(
    ...
    try:
        ...
        async def resolve_one(worker_id: int, rid: int, url: str):
