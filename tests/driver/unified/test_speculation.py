...
async def _enqueue_then_dequeue(req: Request) -> tuple[Request, Request]:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            # seed path, where _ensure_tracked runs first.
            tracking_id = await db.save_speculation_state(
                func_name="fetch_spec:0",
...
async def test_success_advances_and_extends(tmp_path: Path) -> None:
    ...
    await manager.track_outcome(probe, _response(probe, 200))
...
async def test_failure_stops_after_max_gap(tmp_path: Path) -> None:
    ...
    await manager.track_outcome(probe, _response(probe, 404))
...
async def test_probes_point_at_their_tracking_row(tmp_path: Path) -> None:
    ...
    insert fails. This pins the ordering ``_ensure_tracked`` establishes.
    """
    manager, _scraper, db = await _build_manager(tmp_path / "u.db", _SEED)
    await manager.seed()
...
async def test_resume_rehydrates_tracking_id(tmp_path: Path) -> None:
    ...
    # that same row rather than a second one. (The ceiling the *first* seed
    # reached is not persisted until an outcome arrives, so the resumed state
    # extends from the stored 0 — unchanged by this refactor, and irrelevant
    # here beyond guaranteeing the extend does enqueue.)
    probe = _spec_request(scraper, 2, first_id)
    await resumed.track_outcome(probe, _response(probe, 200))
    assert state.current_ceiling > 0
