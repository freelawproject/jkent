...
# The 30 columns ``_deserialize_request`` unpacks, in order.
#
...
# path gives it (and ``HttpMethod(3)`` would raise).
_SELECT = sa.select(
    RequestRow.id,
    RequestRow.request_type,
    RequestRow.method,
    RequestRow.url,
    RequestRow.headers_json,
    RequestRow.cookies_json,
    RequestRow.body,
    RequestRow.continuation,
    RequestRow.current_location,
    RequestRow.accumulated_data_json,
    RequestRow.permanent_json,
    RequestRow.expected_type,
    RequestRow.priority,
    RequestRow.is_speculative,
    RequestRow.speculation_tracking_id,
    RequestRow.speculative_index,
    RequestRow.verify,
    RequestRow.via_json,
    RequestRow.bypass_rate_limit,
    RequestRow.deduplication_key,
    RequestRow.timeout_json,
    RequestRow.json_data,
    RequestRow.files_json,
    RequestRow.auth_json,
    RequestRow.allow_redirects,
    RequestRow.proxies_json,
    RequestRow.stream,
    RequestRow.cert_json,
    RequestRow.archive_hash_header,
    RequestRow.reseedable,
    RequestRow.parent_request_id,
    RequestRow.preresolved,
...
async def _roundtrip(
    ...
    serialized = queue.serialize_request(original).column_values()
    ...
    deserialized = queue._deserialize_request(DequeuedRow(*row))
...
async def test_entry_request_preserves_json_and_extended_fields(
    ...
    deserialized = RequestQueue._deserialize_request(queue, DequeuedRow(*row))
    assert isinstance(deserialized, Request)
