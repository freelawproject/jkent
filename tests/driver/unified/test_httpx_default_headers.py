...
browser transports are covered in ``test_playwright_default_headers``.
"""
...
class _HeaderedScraper(BaseScraper[dict[str, Any]]):
...
def _echo_transport(
...
) -> tuple[HttpxTransport, list[httpx.Headers]]:
    """A mocked transport, and the headers of every request it sends."""
    ...
    return mocked_httpx_transport(echo, scraper), seen
...
async def _wire_headers(
    ...
    transport, seen = _echo_transport(scraper)
    try:
...
class TestDefaultHeaders:
    ...
    async def test_archive_stream_gets_defaults(self) -> None:
        transport, seen = _echo_transport(_HeaderedScraper)
        try:
            handle = await transport.acquire(0)
