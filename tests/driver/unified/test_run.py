...
async def test_worker_death_is_logged_not_swallowed(
    ...
    class _ExplodingWorkerRun(ScrapeRun):
    ...
    try:
        with caplog.at_level(logging.ERROR):
            ...
            with pytest.raises(RuntimeError, match="worker exploded"):
                await task
        assert worker_id not in run._worker_tasks
        assert any(
            "Worker 0 died" in record.message for record in caplog.records
        )
    finally:
        await run.aclose()
...
async def test_compactor_startup_trains_at_threshold(
    ...
    dict_result = await get_compression_dict(
        run._db.session_factory,  # type: ignore[union-attr]
        "parse",
    )
