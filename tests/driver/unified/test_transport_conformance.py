...
archive bodies (``ArchiveStream``). The HTTP and Playwright transports (and
jent's replay transport) implement this one interface.
...
class TransportConformance:
    ...
    async def test_resolve_archive_metadata_and_chunks(
        ...
        The stream is released however the assertions go: a binding whose
        stream is backed by a staged file would otherwise leak it per run.
        """
        await subject.open()
        handle = await subject.acquire(0)
        queued = self.make_queued()
...
class ClassificationConformance:
    ...
    async def _assert_outcome(
        ...
        if outcome == "response":
            ...
            with pytest.raises(PersistentHTTPResponseException) as persistent:
                await self.classify(scraper, code, body)
            self._assert_error_payload(persistent.value, code)

    def _assert_error_payload(self, exc: Any, code: int) -> None:
        """The raised HTTP exception carries what the transport observed.

        The worker persists ``debug_response`` as it would a successful
        response, so a transport that classifies without attaching it
        silently loses the error body/headers from the run db.
        """
        assert exc.status_code == code
        assert exc.debug_response is not None, (
            "the classified exchange must travel on the exception"
        )
        assert exc.debug_response.status_code == code
        assert exc.debug_response.headers, (
            "classified headers must travel on the exception"
        )
        assert CLASSIFY_MARKER in exc.debug_response.content, (
