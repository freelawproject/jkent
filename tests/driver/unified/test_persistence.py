...
async def test_store_response_round_trip(sql_manager: SQLManager) -> None:
    ...
    await storage._store_response(request_id, response, "parse_seed")
...
async def test_handle_retry_backoff_and_ceiling(
    ...
    """_handle_retry returns a sub-ceiling delay, then None once exhausted."""
    request_id = await _seed_request(sql_manager)
    ...
    delay = await storage._handle_retry(request_id, error)
    ...
    saw_none = False
    for _ in range(10):
        result = await storage._handle_retry(request_id, error)
        if result is None:
            saw_none = True
            break
        assert result < 8.0
    assert saw_none, "expected _handle_retry to return None once over ceiling"
