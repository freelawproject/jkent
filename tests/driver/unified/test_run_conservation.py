...
stack (real queue, storage, step — DB-backed) with a counting
...
class DrainScraper(BaseScraper[dict[str, Any]]):
...
class CountingTransport(Transport[NoopHandle]):
    ...
    async def resolve_archive(self, handle: Any, queued: Any) -> Any:
...
async def _status_counts(run: ScrapeRun) -> dict[str, int]:
    ...
    async with sf() as session:
        ...
        return {
            str(RequestStatus.from_code(row[0])): row[1] for row in rows.all()
        }
    ...
    def make_run(
        ...
        *,
        ...
        return ScrapeRun(
            ...
            db_path,
            transport=transport,
            config=replace(config, rate_limited=False),
        )
...
def test_workers_complete_each_request_exactly_once(
    ...
    async def drive() -> tuple[list[int], list[int], dict[str, int], str]:
        ...
        run = run_factory.make_run(
            ...
            transport,
            config=RunConfig(num_workers=workers),
        )
        await run.open()
        try:
