...
Simplifications:
  - Query params are carried in the URL (the queue folds params into the
    URL and does not re-send ``HTTPRequestParams.params``).
  - Generated bodies are raw ``bytes`` (``data``), avoiding json/form
    serialization ambiguity. ``HTTPRequestParams.json`` *is* re-sent by the
    transport (the queue carries it as its own column); a dedicated test below
    covers it directly.
...
from jkent.data_types import HttpMethod, HTTPRequestParams, Request, Selector
...
@pytest.mark.parametrize(
    ...
    [
        ({"a": "1", "b": "2"}, b"a=1&b=2", _FORM),
        # A repeated-field form: order and duplicates as given.
        ([("a", "1"), ("b", "2"), ("a", "3")], b"a=1&b=2&a=3", _FORM),
        (b"raw", b"raw", None),
        (io.BytesIO(b"from a file"), b"from a file", None),
    ],
    ids=["dict", "pair list", "bytes", "file-like"],
)
async def test_every_data_shape_is_sent_on_the_wire(
    data: object, body: bytes, content_type: str | None
) -> None:
    received: dict[str, object] = {}
    ...
    try:
        ...
        assert received["content_type"] == content_type
    finally:
        await transport.aclose()
        await server.runner.cleanup()
...
@pytest.mark.parametrize(
    ...
    [(True, _FORM), (False, None)],
    ids=["form submit", "bare request"],
)
async def test_empty_post_is_urlencoded_only_for_a_form(
    form_post: bool, content_type: str | None
) -> None:
    """A fieldless form POST goes out urlencoded, as a browser sends it.

    Regression: ``Form.submit`` on a form with nothing to submit yields
    ``data == {}``, which httpx sends with no Content-Type at all (the
    browser still says urlencoded, and a server may 415 the bare POST). A
    scraper's own empty POST is left as httpx sends it — the queue stores
    ``{}`` and ``None`` alike, so only a form via tells the two apart.
    """
    received: dict[str, object] = {}
    ...
    if form_post:
        doc = lxml_html.fromstring(
            f'<form id="f" method="post" action="{url}"></form>'
        )
        form = LxmlPageElement(doc, url).find_form(Selector.CSS("#f"), "form")
        request = form.submit(step="parse")
        assert request.request.data == {}
    else:
        request = Request(
            request=HTTPRequestParams(
                method=HttpMethod.POST, url=url, data={}
            ),
            step="parse",
        )
    transport = HttpxTransport()
