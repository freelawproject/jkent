...
CREATE INDEX idx_requests_status_priority ON requests (status, priority, id);

-- index: idx_requests_step
...
CREATE TABLE errors (
	...
	CONSTRAINT ck_errors_kind CHECK (kind IN (1, 2, 3, 4, 5, 6, 7, 8)),
...
CREATE TABLE incidental_request_storage (
	...
	PRIMARY KEY (id),
	CONSTRAINT uq_irs_content_md5 UNIQUE (content_md5),
...
CREATE TABLE requests (
	...
	body BLOB,
	body_is_form BOOLEAN DEFAULT 0 NOT NULL,
	step VARCHAR NOT NULL,
	...
	via_json VARCHAR,
	verify_json VARCHAR DEFAULT 'true' NOT NULL,
	...
	CONSTRAINT ck_requests_status CHECK (status IN (1, 2, 3, 4, 6)),
	...
	CONSTRAINT ck_requests_speculation_outcome CHECK (speculation_outcome IN (1, 2, 3, 4)),
	...
	CONSTRAINT ck_requests_verify_json_valid CHECK (json_valid(verify_json)),
	FOREIGN KEY(parent_request_id) REFERENCES requests (id) ON DELETE CASCADE,
...
CREATE TABLE speculation_tracking (
	...
	highest_successful_id INTEGER,
	consecutive_failures INTEGER DEFAULT 0 NOT NULL,
	current_ceiling INTEGER,
