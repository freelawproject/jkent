...
Small wrappers over ``select()`` on the ORM models, used where tests
...
async def fetch_requests(
    ...
    """All request rows, optionally filtered by status label, in id order."""
...
async def fetch_results(
    ...
    if request_id is not None:
        stmt = stmt.where(Result.request_id == request_id)
    if is_valid is not None:
        stmt = stmt.where(Result.is_valid == is_valid)
    async with sql_manager.session_factory() as session:
        return list((await session.execute(stmt)).scalars().all())
