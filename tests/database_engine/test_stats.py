    ...
    async with sql_manager.session_factory() as session:
        result = await session.execute(
            sa.text(
                ...
                "VALUES (:status, 9, :qc, :method, 'https://s', :cont, '', "
...
async def test_run_summary_aggregates(sql_manager: SQLManager) -> None:
    ...
    assert (None, "unknown", 1) in summary.errors_by_continuation
    assert ("detail", "transient", 1) in summary.errors_by_continuation
    assert ("save_pdf", "persistent", 1) in summary.errors_by_continuation
