...
class TestIncidentalRequestStorage:
    async def test_insert_creates_both_rows(
        ...
        assert srow[1] == "GET"
        assert srow[3] == hashlib.md5(compressed).digest()
    ...
    async def test_get_incidental_request_storage(
        ...
        # get_incidental_request_storage hexes the digest for its JSON-bound
        # payload; the column itself holds the raw bytes.
...
class TestIncidentalRequestRecord:
    ...
    def test_to_dict(self) -> None:
        r = IncidentalRequestRecord(
            ...
            from_cache=False,
            created_at=datetime(2024, 1, 1, 12, 30, 45, 123000),
            storage_id=5,
            resource_type="script",
            method="GET",
            status_code=200,
        ...
        # Serialized, not the datetime itself — to_json() has to survive it.
        assert d["created_at"] == "2024-01-01T12:30:45.123000"
