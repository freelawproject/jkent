...
* ``ANY /echo`` — the form ``action``. It parses the submission in *received
  ...
  body) and echoes it back, with the body's content type, inside
  ``<pre id="echo">…</pre>`` as JSON. A body that is not urlencoded echoes
  no pairs, so a transport that sent JSON or multipart diverges on the
  content type rather than on a strange key parsed out of it. Embedding
...
# A canonical submission: the method, the body's content type (``""`` for a
# GET), and the ordered (name, value) pairs the server received.
Canonical = tuple[str, str, tuple[tuple[str, str], ...]]
...
async def _echo(request: web.Request) -> web.Response:
    if request.method == "GET":
        content_type = ""
        pairs: list[tuple[str, str]] = parse_qsl(
            request.query_string, keep_blank_values=True
        )
    else:
        # The mime type alone: a multipart boundary is per-request noise.
        content_type = request.content_type
        raw = await request.text()
        pairs = (
            parse_qsl(raw, keep_blank_values=True)
            if content_type == _URLENCODED
            else []
        )
    payload = json.dumps(
        {
...
def extract_echo(html_text: str) -> Canonical:
    ...
    return (str(payload["method"]), str(payload["content_type"]), pairs)
...
def format_canonical(label: str, canonical: Canonical | str) -> str:
    ...
    method, content_type, pairs = canonical
    lines = [f"  {label}: {method} {content_type}".rstrip()]
