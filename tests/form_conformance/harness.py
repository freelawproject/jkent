...
from tests.form_conformance.model import CHECKBOXES
...
#: Everything the rig launches besides httpx (always up). A rig that lost one
#: of these would still go green, comparing fewer transports than it claims.
BROWSERS: Final = frozenset({"chromium", "firefox", "oracle"})
...
class Harness:
    ...
    def __init__(
        ...
        *,
        required: frozenset[str] = BROWSERS,
    ) -> None:
        """``required``: the :data:`BROWSERS` whose launch failure fails ``open``.

        The rest are best-effort: a failed launch drops them from the
        comparison (see :attr:`available`, :attr:`oracle_available`).
        """
        self._loop = loop
        ...
        self.firefox: PlaywrightTransport | None = None
    ...
    async def open(self) -> None:
        ...
        # Browser transports — required unless the caller said otherwise.
        self.chromium = PlaywrightTransport(
            _Scraper(), headless=True, browser_type="chromium", db=self.sql
        )
        self.available["chromium"] = await self._try_open(
            "chromium", self.chromium
        )

        self.firefox = PlaywrightTransport(
            _Scraper(), headless=True, browser_type="camoufox", db=self.sql
        )
        self.available["firefox"] = await self._try_open(
    ...
    async def _open_oracle(self) -> None:
        try:
            # Guarded: playwright may not be installed in the environment.
            # Unless the oracle is required, the bound tests then skip.
            ...
            if "oracle" in self._required:
                raise RuntimeError("required oracle did not launch") from exc

    async def _try_open(self, name: str, transport: Any) -> bool:
        """Open ``transport``, returning whether it launched (closing on failure).

        A required transport's launch failure propagates instead.
        """
        try:
            ...
            if name in self._required:
                raise RuntimeError(
                    f"required transport {name!r} did not launch"
                ) from exc
            return False
        return True
    ...
    async def _guard(coro: Any) -> Canonical | str:
        ...
        try:
            return await coro
        except Exception as exc:  # surfaced in the diff
    ...
    async def _submit_oracle(self, case: FormCase, action: str) -> Canonical:
        ...
        try:
            ...
            for selector, kind, checked in case.native_checks():
                if checked or kind == CHECKBOXES:
                    await page.set_checked(selector, checked)
                else:
                    # A radio can't be unchecked by a click; clearing it is
                    # how a group ends up submitting none of its boxes.
                    await page.eval_on_selector(
                        selector, "(el) => { el.checked = false; }"
                    )
            for selector, values in case.native_selects():
    ...
    async def _submit_playwright(
        ...
        parent_id = await insert_staged_parent(
            self.sf, url=parent_url, body=rendered.encode()
        )
    ...
    async def _stage_and_dequeue(
        ...
        :meth:`RequestQueue.get_next_request` runs, and the row dequeued is
        the one inserted. The dequeue is paired with ``request_done`` as a
        worker's is, so the queue's in-flight count does not climb across
        submissions.
        """
        # Bind to locals: attribute narrowing is invalidated across awaits.
        queue = self.queue
        ...
        request_data = queue.serialize_request(request)
        request_data.priority = request.effective_priority
        request_data.parent_request_id = parent_id
        inserted = await sql.insert_request(request_data)
        assert inserted.inserted, "staged request collided with an earlier one"
        assert await sql.count_pending_requests() == 1
        dequeued = await queue.get_next_request()
        queue.request_done()
        assert dequeued is not None, "staged request was not dequeued"
        request_id, staged, parent, _preresolved = dequeued
        assert request_id == inserted.request_id
        assert queue.in_flight_count == 0
        return request_id, staged, parent
