...
* ``native_fills`` / ``native_checks`` / ``native_selects`` /
  ``submit_selector`` — the high-level actions the oracle browser performs
  (type into a field, check or uncheck a box, pick select options, click the
  chosen submit) so its submission expresses the *same intent*
  as the transports' request.

Equivalence is kept airtight by construction (see the per-kind notes below):
by default checkbox/radio/select state lives in the *rendered* markup
(document order, no override) so ordering matches a browser; only
genuinely-empty text fields are "filled" via an override that replaces an
existing key in place. Where a divergence is expected by design (extra
fields, non-first submit buttons, group overrides) the differential tests
...
class Control:
    ...
    def override(self) -> dict[str, str]:
        ...
        override; disabled controls contribute nothing. A radio/checkbox
        group's override is ``FormCase.group_overrides``, a select's is
        ``FormCase.select_overrides``.
        """
...
class FormCase:
    ...
    # ``Form.submit(data=)`` overrides for a radio/checkbox group:
    # ``name -> value`` (scalar) or ``name -> (value, …)`` (list, possibly
    # empty). The group submits exactly these values: the oracle checks the
    # boxes carrying them, unchecks the rest, and sends a value no box
    # carries as a real hidden input, as it does for ``extras``.
    group_overrides: tuple[tuple[str, str | tuple[str, ...]], ...] = ()
    # ``Form.submit(data=)`` overrides for a ``<select>`` (scalar or list, as
    # above). The oracle selects exactly these options; a value no option
    # carries is first added to the select as a real ``<option>``, as the
    # Playwright fill path injects one.
    select_overrides: tuple[tuple[str, str | tuple[str, ...]], ...] = ()
    ...
    def oracle_html(self, action: str) -> str:
        ...
        """
        body = "".join(c.render() for c in self._oracle_controls())
        hidden = [*self.extras, *self._unmatched_group_values()]
        body += "".join(
    ...
    def _group(self, name: str) -> Control:
        return next(c for c in self.controls if c.name == name)

    @staticmethod
    def _wanted(value: str | tuple[str, ...]) -> tuple[str, ...]:
        return (value,) if isinstance(value, str) else value

    def _unmatched_group_values(self) -> list[tuple[str, str]]:
        """Group-override values no rendered box carries, as (name, value)."""
        return [
            (name, v)
            for name, value in self.group_overrides
            for v in self._wanted(value)
            if v not in self._group(name).options
        ]
    ...
    def overrides(self) -> dict[str | Selector, str | list[str | None]]:
        ...
        for name, group_value in (
            *self.group_overrides,
            *self.select_overrides,
        ):
            if isinstance(group_value, str):
                data[name] = group_value
            else:
                data[name] = list[str | None](group_value)
    ...
    def _oracle_controls(self) -> list[Control]:
        """The controls, each overridden select given an ``<option>`` for
        every override value it lacks (appended, as the fill path does)."""
        overrides = dict(self.select_overrides)
        out: list[Control] = []
        for c in self.controls:
            if c.name in overrides and c.kind in (SELECT, MULTISELECT):
                extra = tuple(
                    v
                    for v in self._wanted(overrides[c.name])
                    if v not in c.options
                )
                c = replace(c, options=(*c.options, *extra))
            out.append(c)
        return out

    def native_selects(self) -> list[tuple[str, list[str]]]:
        """``(selector, values)`` the oracle selects, one per overridden select.

        An empty list deselects everything (a multi-select only).
        """
        return [
            (f"#{self._group(name).elem_id}", list(self._wanted(value)))
            for name, value in self.select_overrides
        ]

    def native_checks(self) -> list[tuple[str, str, bool]]:
        """``(selector, kind, checked)`` for every box in an overridden group.

        The oracle sets each box to whether its value is one the override
        submits; boxes of groups without an override are left as rendered.
        """
        out: list[tuple[str, str, bool]] = []
        for name, value in self.group_overrides:
            group = self._group(name)
            wanted = self._wanted(value)
            for j, option in enumerate(group.options):
                out.append(
                    (f"#{group.elem_id}_{j}", group.kind, option in wanted)
                )
        return out
