...
def test_reentering_same_observer_unwinds_cleanly():
    """Re-entering an already-active observer nests without clobbering.

    The @step wrapper re-activates its observer around every generator
    resume; when that observer was injected by an outer ``with`` block, the
    inner activations are re-entries of the same instance and must not steal
    the outer activation's contextvar token.
    """
    with SelectorObserver() as observer:
        with observer:
            assert get_active_observer() is observer
        # Inner exit leaves the outer activation in force.
        assert get_active_observer() is observer
...
def test_json_output_nested_has_no_parent_cycle(simple_html):
    ...
    # A scoped query on a matched row gives its query a non-None parent and
    # appends it to the parent's children -- the shape that used to cycle.
    cells = rows[0].xpath(".//td")
    observer.record_query(
        selector=".//td",
        selector_type="xpath",
