...
@icontract.ensure(
    lambda result: result[0] == result[1],
    "replay-index fallback serialization agrees with the queue's",
)
def replay_body_agrees_with_queue_body(
    ...
    The index-build side hashes the queue's stored (url, body); the
    lookup side re-serializes the yielded request. Both must produce
    the same bytes or replay misses. Guards a fixed bug at ``form={}``:
    the queue stores None for falsy data, but the index side only
    checked ``is None`` and serialized ``b"{}"`` — diverging keys.
    """
    ...
    stored = RequestQueueDB()._serialize_request(request)
    queue_side = (stored["url"], stored["body"])
    index_side = serialize_url_and_body(http_request)
    return (queue_side, index_side)
...
def _canonical_body(value: object) -> object:
    """A body in the form the queue is specified to preserve it.

    The queue privileges JSON: its loader decodes any bytes body that
    parses as JSON (queue.py "Decode body" branch) and drops empty
    bodies (its ``if body:`` truthiness check). So: empty/None → None;
    JSON-shaped bytes and already-decoded values → canonical JSON text
    (``sort_keys`` for dict-order stability, text form so ``NaN``
    compares equal to itself); any other bytes → the exact bytes.
    """
...
def queue_body_round_trips(data: bytes) -> tuple[object, object]:
    ...
    everything else. Both sides go through :func:`_canonical_body`
    before comparison.
    """
    request = Request(
    ...
    stored = queue._serialize_request(request)
    ...
    return (
        _canonical_body(request.request.data),
        _canonical_body(restored.request.data),
    )
