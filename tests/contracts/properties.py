...
# (queue.py): the RequestInsert fields in DequeuedRow column order, with
# id/priority/dedup_key interleaved where the row puts them.
...
def queue_body_round_trips(data: bytes) -> tuple[object, object]:
    ...
    row_fields += [getattr(stored, f) for f in _ROW_FIELDS_AFTER_URL]
    # priority lives on the row, not the serialized payload
    row_fields += [
        0,
        stored.is_speculative,
        stored.speculation_tracking_id,
        stored.speculative_index,
    ]
    row_fields += [getattr(stored, f) for f in _ROW_FIELDS_AFTER_SPECULATION]
    row_fields.append(request.deduplication_key)
    row_fields += [getattr(stored, f) for f in _ROW_FIELDS_TAIL]
    # The queue-state tail of the row: no longer defaulted on
    # ``DequeuedRow``, so it is spelled out here.
    row_fields += [stored.parent_id, False]
    restored = queue._deserialize_request(
        DequeuedRow(*row_fields)  # type: ignore[arg-type]
    )
    return (
